...
export default function ThreadUpdateForm(props) {
  const {
    id: idProp,
    thread: threadModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = threadRecord
      ? { ...initialValues, ...threadRecord, posts: linkedPosts }
      : initialValues;
  ...
  const [threadRecord, setThreadRecord] = React.useState(threadModelProp);
  const [linkedPosts, setLinkedPosts] = React.useState([]);
  const canUnlinkPosts = false;
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Thread, idProp)
        : threadModelProp;
      setThreadRecord(record);
      const linkedPosts = record ? await record.posts.toArray() : [];
      setLinkedPosts(linkedPosts);
    };
    queryData();
  }, [idProp, threadModelProp]);
  React.useEffect(resetStateValues, [threadRecord, linkedPosts]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          postsToUnLink.forEach((original) => {
            if (!canUnlinkPosts) {
              throw Error(
                `Post ${original.id} cannot be unlinked from Thread because oblogID is a required field.`
              );
            }
            promises.push(
              DataStore.save(
                Post.copyOf(original, (updated) => {
                  updated.oblogID = null;
                })
              )
            );
          });
          postsToLink.forEach((original) => {
            promises.push(
              DataStore.save(
                Post.copyOf(original, (updated) => {
                  updated.oblogID = threadRecord.id;
                })
              )
            );
          });
          ...
          promises.push(
            DataStore.save(
              Thread.copyOf(threadRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ThreadUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || threadModelProp)}
