...
export default function TaskSetCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setTaskItems(initialValues.TaskItems);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const taskSet = await DataStore.save(new TaskSet(modelFieldsToSave));
          const promises = [];
          promises.push(
            ...TaskItems.reduce((promises, original) => {
              promises.push(
                DataStore.save(
                  TaskItem.copyOf(original, (updated) => {
                    updated.tasksetID = taskSet.id;
                  })
                )
              );
              return promises;
      ...
      {...getOverrideProps(overrides, "TaskSetCreateForm")}
      {...rest}
    >
      <ArrayField
