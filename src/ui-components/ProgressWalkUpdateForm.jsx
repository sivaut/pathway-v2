...
export default function ProgressWalkUpdateForm(props) {
  const {
    id: idProp,
    progressWalk: progressWalkModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = progressWalkRecord
      ? {
          ...initialValues,
          ...progressWalkRecord,
          progresspathID,
          definedWalk,
          hasProgressSteps: linkedHasProgressSteps,
        }
      : initialValues;
    setProgresspathID(cleanValues.progresspathID);
    setCurrentProgresspathIDValue(undefined);
    setCurrentProgresspathIDDisplayValue("");
    setDefinedWalk(cleanValues.definedWalk);
    setCurrentDefinedWalkValue(undefined);
    setCurrentDefinedWalkDisplayValue("");
    setHasProgressSteps(cleanValues.hasProgressSteps ?? []);
  ...
  const [progressWalkRecord, setProgressWalkRecord] = React.useState(
    progressWalkModelProp
  );
  const [linkedHasProgressSteps, setLinkedHasProgressSteps] = React.useState(
    []
  );
  const canUnlinkHasProgressSteps = false;
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ProgressWalk, idProp)
        : progressWalkModelProp;
      setProgressWalkRecord(record);
      const progresspathIDRecord = record
        ? await record.progresspathID
        : undefined;
      setProgresspathID(progresspathIDRecord);
      const definedWalkRecord = record ? await record.definedWalk : undefined;
      setDefinedWalk(definedWalkRecord);
      const linkedHasProgressSteps = record
        ? await record.hasProgressSteps.toArray()
        : [];
      setLinkedHasProgressSteps(linkedHasProgressSteps);
    };
    queryData();
  }, [idProp, progressWalkModelProp]);
  React.useEffect(resetStateValues, [
    progressWalkRecord,
    progresspathID,
    definedWalk,
    linkedHasProgressSteps,
  ]);
  const [
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const hasProgressStepsToLink = [];
          const hasProgressStepsToUnLink = [];
          const hasProgressStepsSet = new Set();
          const linkedHasProgressStepsSet = new Set();
          hasProgressSteps.forEach((r) =>
            hasProgressStepsSet.add(getIDValue.hasProgressSteps?.(r))
          );
          linkedHasProgressSteps.forEach((r) =>
            linkedHasProgressStepsSet.add(getIDValue.hasProgressSteps?.(r))
          );
          linkedHasProgressSteps.forEach((r) => {
            if (!hasProgressStepsSet.has(getIDValue.hasProgressSteps?.(r))) {
              hasProgressStepsToUnLink.push(r);
            }
          });
          hasProgressSteps.forEach((r) => {
            if (
              !linkedHasProgressStepsSet.has(getIDValue.hasProgressSteps?.(r))
            ) {
              hasProgressStepsToLink.push(r);
            }
          });
          hasProgressStepsToUnLink.forEach((original) => {
            if (!canUnlinkHasProgressSteps) {
              throw Error(
                `ProgressStep ${original.id} cannot be unlinked from ProgressWalk because progresswalkID is a required field.`
              );
            }
            promises.push(
              DataStore.save(
                ProgressStep.copyOf(original, (updated) => {
                  updated.progresswalkID = null;
                })
              )
            );
          });
          hasProgressStepsToLink.forEach((original) => {
            promises.push(
              DataStore.save(
                ProgressStep.copyOf(original, (updated) => {
                  updated.progresswalkID = progressWalkRecord.id;
                })
              )
            );
          });
          ...
          promises.push(
            DataStore.save(
              ProgressWalk.copyOf(progressWalkRecord, (updated) => {
                Object.assign(updated, modelFieldsToSave);
                if (!modelFieldsToSave.definedWalk) {
                  updated.progressWalkDefinedWalkId = undefined;
                }
              })
            )
          );
      ...
      {...getOverrideProps(overrides, "ProgressWalkUpdateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || progressWalkModelProp)}
