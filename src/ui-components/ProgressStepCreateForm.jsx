...
export default function ProgressStepCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const progressStep = await DataStore.save(
            new ProgressStep(modelFieldsToSave)
          );
          const promises = [];
          promises.push(
            ...TaskItemExecutions.reduce((promises, original) => {
              promises.push(
                DataStore.save(
                  TaskItemExecution.copyOf(original, (updated) => {
                    updated.progressstepID = progressStep.id;
                  })
                )
              );
              return promises;
      ...
      {...getOverrideProps(overrides, "ProgressStepCreateForm")}
      {...rest}
    >
      <SwitchField
        label="Status"
