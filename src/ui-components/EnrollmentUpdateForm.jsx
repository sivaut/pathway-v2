...
export default function EnrollmentUpdateForm(props) {
  ...
  React.useEffect(() => {
    const queryData = async () => {
      ...
      const toPathwayRecord = record ? await record.toPathway : undefined;
      setToPathway(toPathwayRecord);
      const tPathwayVersionoRecord = record
        ? await record.tPathwayVersiono
        : undefined;
      setTPathwayVersiono(tPathwayVersionoRecord);
      const profileexplorerIDRecord = record
        ? await record.profileexplorerID
        : undefined;
      setProfileexplorerID(profileexplorerIDRecord);
      const ProgressPathRecord = record ? await record.ProgressPath : undefined;
      setProgressPath(ProgressPathRecord);
    };
    queryData();
        ...
        try {
          ...
          await DataStore.save(
            Enrollment.copyOf(enrollmentRecord, (updated) => {
              ...
              if (!modelFields.toPathway) {
                updated.enrollmentToPathwayId = undefined;
              }
              if (!modelFields.tPathwayVersiono) {
                updated.enrollmentTPathwayVersionoId = undefined;
              }
              if (!modelFields.ProgressPath) {
                updated.enrollmentProgressPathId = undefined;
              }
            })
          );
          if (onSuccess) {
    ...
    >
      ...
      >
        <Autocomplete
          ...
          defaultValue={profileexplorerID}
          onChange={(e) => {
            let { value } = e.target;
