...
export default function WalkCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setPathwayversionID(initialValues.pathwayversionID);
    ...
    setWalkTags(initialValues.WalkTags);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const walk = await DataStore.save(new Walk(modelFieldsToSave));
          const promises = [];
          promises.push(
            ...Steps.reduce((promises, original) => {
              promises.push(
                DataStore.save(
                  Step.copyOf(original, (updated) => {
                    updated.walkID = walk.id;
                  })
                )
              );
              return promises;
          ...
          promises.push(
            ...WalkTags.reduce((promises, walkTag) => {
              promises.push(
                DataStore.save(
                  new WalkWalkTag({
                    walk,
      ...
      {...getOverrideProps(overrides, "WalkCreateForm")}
      {...rest}
    >
      <ArrayField
