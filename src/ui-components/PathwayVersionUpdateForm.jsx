...
export default function PathwayVersionUpdateForm(props) {
  const {
    id: idProp,
    pathwayVersion: pathwayVersionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = pathwayVersionRecord
      ? {
          ...initialValues,
          ...pathwayVersionRecord,
          ofPathway,
          Walks: linkedWalks,
        }
      : initialValues;
    setOfPathway(cleanValues.ofPathway);
    ...
    setLatest(cleanValues.latest);
    setErrors({});
  };
  const [pathwayVersionRecord, setPathwayVersionRecord] = React.useState(
    pathwayVersionModelProp
  );
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(PathwayVersion, idProp)
        : pathwayVersionModelProp;
      setPathwayVersionRecord(record);
      const ofPathwayRecord = record ? await record.ofPathway : undefined;
      setOfPathway(ofPathwayRecord);
      const linkedWalks = record ? await record.Walks.toArray() : [];
      setLinkedWalks(linkedWalks);
    };
    queryData();
  }, [idProp, pathwayVersionModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const pathwayToUnlink = await pathwayVersionRecord.ofPathway;
          if (pathwayToUnlink) {
            promises.push(
              DataStore.save(
            ...
            promises.push(
              DataStore.save(
                Pathway.copyOf(pathwayToLink, (updated) => {
                  updated.hasManyVersion = pathwayVersionRecord;
                })
              )
            );
          ...
          const walksToLink = [];
          const walksToUnLink = [];
          const walksSet = new Set();
          const linkedWalksSet = new Set();
          Walks.forEach((r) => walksSet.add(getIDValue.Walks?.(r)));
          linkedWalks.forEach((r) => linkedWalksSet.add(getIDValue.Walks?.(r)));
          linkedWalks.forEach((r) => {
            if (!walksSet.has(getIDValue.Walks?.(r))) {
              walksToUnLink.push(r);
            }
          });
          Walks.forEach((r) => {
            if (!linkedWalksSet.has(getIDValue.Walks?.(r))) {
              walksToLink.push(r);
            }
          });
          walksToUnLink.forEach((original) => {
            if (!canUnlinkWalks) {
              throw Error(
                `Walk ${original.id} cannot be unlinked from PathwayVersion because pathwayversionID is a required field.`
              );
            }
            promises.push(
              DataStore.save(
                Walk.copyOf(original, (updated) => {
                  updated.pathwayversionID = null;
                })
              )
            );
          });
          walksToLink.forEach((original) => {
            promises.push(
              DataStore.save(
                Walk.copyOf(original, (updated) => {
                  updated.pathwayversionID = pathwayVersionRecord.id;
                })
              )
            );
          });
          ...
          promises.push(
            DataStore.save(
              PathwayVersion.copyOf(pathwayVersionRecord, (updated) => {
                Object.assign(updated, modelFieldsToSave);
                if (!modelFieldsToSave.ofPathway) {
                  updated.pathwayVersionOfPathwayId = undefined;
                }
              })
            )
      ...
      {...getOverrideProps(overrides, "PathwayVersionUpdateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || pathwayVersionModelProp)}
