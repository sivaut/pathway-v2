...
export default function WalkTagCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const walkTag = await DataStore.save(new WalkTag(modelFieldsToSave));
          const promises = [];
          promises.push(
            ...walks.reduce((promises, walk) => {
              promises.push(
                DataStore.save(
                  new WalkWalkTag({
                    walkTag,
      ...
      {...getOverrideProps(overrides, "WalkTagCreateForm")}
      {...rest}
    >
      <TextField
        label="Name"
