...
export default function StepUpdateForm(props) {
  const {
    id: idProp,
    step: stepModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = stepRecord
      ? { ...initialValues, ...stepRecord, walkID, TaskSet }
      : initialValues;
    setWalkID(cleanValues.walkID);
    setCurrentWalkIDValue(undefined);
    setCurrentWalkIDDisplayValue("");
    setTaskSet(cleanValues.TaskSet);
    ...
    setSequence(cleanValues.sequence);
    setType(cleanValues.type);
    setErrors({});
  };
  const [stepRecord, setStepRecord] = React.useState(stepModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Step, idProp)
        : stepModelProp;
      setStepRecord(record);
      const walkIDRecord = record ? await record.walkID : undefined;
      setWalkID(walkIDRecord);
      const TaskSetRecord = record ? await record.TaskSet : undefined;
      setTaskSet(TaskSetRecord);
    };
    queryData();
  }, [idProp, stepModelProp]);
  React.useEffect(resetStateValues, [stepRecord, walkID, TaskSet]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Step.copyOf(stepRecord, (updated) => {
              Object.assign(updated, modelFields);
              if (!modelFields.TaskSet) {
                updated.stepTaskSetId = undefined;
              }
            })
          );
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "StepUpdateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || stepModelProp)}
