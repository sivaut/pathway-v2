...
export default function PathwayTagCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setPathways(initialValues.pathways);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const pathwayTag = await DataStore.save(
            new PathwayTag(modelFieldsToSave)
          );
          const promises = [];
          promises.push(
            ...pathways.reduce((promises, pathway) => {
              promises.push(
                DataStore.save(
      ...
      {...getOverrideProps(overrides, "PathwayTagCreateForm")}
      {...rest}
    >
      <TextField
        label="Name"
