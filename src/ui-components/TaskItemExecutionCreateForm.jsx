...
export default function TaskItemExecutionCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setTaskItem(initialValues.TaskItem);
    setCurrentTaskItemValue(undefined);
    setCurrentTaskItemDisplayValue("");
    setType(initialValues.type);
    setStatus(initialValues.status);
    setProgressstepID(initialValues.progressstepID);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(new TaskItemExecution(modelFields));
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "TaskItemExecutionCreateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
